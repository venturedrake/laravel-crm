<div class="crm-content">
    ...
    <x-mary-card shadow>
        <x-mary-table :headers="$headers" :rows="$purchaseOrders" link="/purchase-orders/{id}" with-pagination :sort-by="$sortBy" class="whitespace-nowrap">
            @scope('cell_labels', $purchaseOrder)
                @foreach($purchaseOrder->labels as $label)
            ...
            <x-mary-button onclick="modalDeleteLead{{ $purchaseOrder->id }}.showModal()" icon="o-trash" class="btn-sm btn-square btn-error text-white" spinner />
            <x-crm-delete-confirm model="purchase_order" id="{{ $purchaseOrder->id }}" />
            @endscope
        </x-mary-table>
    </x-mary-card>
