<div class="crm-content"
    ...
    {{-- HEADER --}}
    <x-mary-header title="{{ ucfirst(__('laravel-crm::lang.import_people')) }}" class="mb-5" progress-indicator>
        <x-slot:actions>
            <x-mary-button
                label="{{ ucfirst(__('laravel-crm::lang.download_sample_csv')) }}"
                link="{{ url(route('laravel-crm.people.import.sample')) }}"
                icon="o-arrow-down-tray"
                class="btn-sm btn-outline"
                responsive />
            <x-mary-button
                label="{{ ucfirst(__('laravel-crm::lang.back_to_people')) }}"
                link="{{ url(route('laravel-crm.people.index')) }}"
        ...
        <x-mary-card shadow class="mb-5">
            <div class="flex flex-col items-center gap-4 py-8 text-center">
                ...
                <p class="text-base-content/70">
                    <span class="font-semibold text-success">{{ $importedCount }}</span> {{ __('laravel-crm::lang.people_imported_successfully') }},
                ...
                <div class="flex gap-3 mt-2">
                    ...
                    <x-mary-button label="{{ ucfirst(__('laravel-crm::lang.view_people')) }}" link="{{ url(route('laravel-crm.people.index')) }}" icon="o-users" class="btn-outline" />
                </div>
            </div>
        </x-mary-card>
    ...
    @elseif($hasPreview && count($previewRows) > 0)
        @php
            $allRows     = session('crm_person_import_preview', []);
        ...
        <x-mary-card shadow class="mb-5" title="{{ ucfirst(__('laravel-crm::lang.preview')) }} ({{ $totalRows }} {{ __('laravel-crm::lang.rows') }})" separator>
            <div class="overflow-x-auto">
                <table class="table table-sm table-zebra w-full">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>{{ ucfirst(__('laravel-crm::lang.first_name')) }}</th>
                            <th>{{ ucfirst(__('laravel-crm::lang.last_name')) }}</th>
                            <th>{{ ucfirst(__('laravel-crm::lang.email')) }}</th>
                            <th>{{ ucfirst(__('laravel-crm::lang.phone')) }}</th>
                            <th>{{ ucfirst(__('laravel-crm::lang.organization')) }}</th>
            ...
            <div class="flex items-center justify-between mt-4 pt-4 border-t border-base-300">
                ...
                <div class="flex items-center gap-3">
                    @if($validRows > 0)
                        <x-mary-button
                            label="{{ ucfirst(__('laravel-crm::lang.import_people')) }} ({{ $validRows }})"
                            onclick="modalImportPeople.showModal()"
                            icon="o-arrow-up-tray"
                            class="btn-primary text-white" />

                        <dialog id="modalImportPeople" class="modal">
                            <div class="modal-box text-left">
                                <h3 class="text-lg font-bold">{{ ucfirst(__('laravel-crm::lang.import_people')) }}?</h3>
                                <p class="py-4">{{ __('laravel-crm::lang.import_people_confirm', ['count' => $validRows]) }}</p>
                                <div class="modal-action">
                                    <form method="dialog">
                                        <button class="btn">{{ ucfirst(__('laravel-crm::lang.cancel')) }}</button>
                                        <button wire:click="startImport" class="btn btn-primary text-white">{{ ucfirst(__('laravel-crm::lang.import_people')) }}</button>
                                    </form>
                                </div>
                            </div>
                        </dialog>
    ...
    @else
        <x-mary-card shadow class="mb-5" title="{{ ucfirst(__('laravel-crm::lang.upload_csv_file')) }}" separator>
            <div class="grid gap-4">
                ...
                <div class="alert alert-info text-sm">
                    ...
                    <div>
                        ...
                        <ul class="list-disc list-inside space-y-0.5">
                            <li><code>first_name</code> — {{ __('laravel-crm::lang.import_col_first_name') }}</li>
                        </ul>
                        ...
                        <ul class="list-disc list-inside space-y-0.5">
                            ...
                            <li><code>organization_name</code> — {{ __('laravel-crm::lang.import_col_organization_name') }}</li>
                ...
                <form method="POST" action="{{ route('laravel-crm.people.import.parse') }}" enctype="multipart/form-data">
                    @csrf
