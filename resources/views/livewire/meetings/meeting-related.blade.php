<div class="grid gap-5">
    <x-mary-card title="{{ ucfirst(__('laravel-crm::lang.add_meeting')) }}" separator>
                ...
                <div class="grid gap-3">
                    <div class="flex justify-between items-start">
                        ...
                        <x-mary-dropdown right top>
                            ...
                            <x-mary-menu-item wire:click="edit({{ $meeting->id }})" title="{{ ucfirst(__('laravel-crm::lang.edit')) }}" />
                            <x-mary-menu-item onclick="modalDeleteMeeting{{ $meeting->id }}.showModal()" title="{{ ucfirst(__('laravel-crm::lang.delete')) }}" />
                        </x-mary-dropdown>
                    </div>

                    @if(! empty($data[$meeting->id]['editing']))
                        <x-mary-form wire:submit="update({{ $meeting->id }})">
                            <div class="grid gap-3" wire:key="meeting-update-{{ $meeting->id }}">
                                <x-mary-input wire:model="data.{{ $meeting->id }}.name" label="{{ ucfirst(__('laravel-crm::lang.subject')) }}" />
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" x-data="{
                                    snap15(el) {
                                        if (!el.value) return;
                                        const d = new Date(el.value);
                                    ...
                                    <x-mary-datetime wire:model="data.{{ $meeting->id }}.start_at" label="{{ ucfirst(__('laravel-crm::lang.start_at')) }}" type="datetime-local" x-on:change="snap15($event.target)" />
                                    <x-mary-datetime wire:model="data.{{ $meeting->id }}.finish_at" label="{{ ucfirst(__('laravel-crm::lang.finish_at')) }}" type="datetime-local" x-on:change="snap15($event.target)" />
                                </div>
                                <x-mary-choices-offline
                                        wire:model="data.{{ $meeting->id }}.guests"
                                        label="{{ ucfirst(__('laravel-crm::lang.guests')) }}"
                                        :options="$persons"
                                        placeholder="Search ..."
                                        searchable />
                                <x-mary-input wire:model="data.{{ $meeting->id }}.location" label="{{ ucfirst(__('laravel-crm::lang.location')) }}" />
                                <x-mary-textarea wire:model="data.{{ $meeting->id }}.description" label="{{ ucfirst(__('laravel-crm::lang.description')) }}" rows="5" />
                            </div>
                            <x-slot:actions>
                                <x-mary-button wire:click="cancel({{ $meeting->id }})" label="{{ ucfirst(__('laravel-crm::lang.cancel')) }}" type="button" />
                                <x-mary-button wire:click="update({{ $meeting->id }})" label="{{ ucfirst(__('laravel-crm::lang.save_changes')) }}" class="btn-primary text-white" type="button" spinner="update({{ $meeting->id }})" />
                            </x-slot:actions>
                        </x-mary-form>
                    @else
                        <hr />
                        ...
                        {{ $meeting->description }}
                        <x-crm-delete-confirm model="meeting" id="{{ $meeting->id }}" />
                    @endif
                </div>
            </x-mary-card>
        @endforeach
