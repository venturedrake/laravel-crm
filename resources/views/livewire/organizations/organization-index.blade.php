            ...
            @scope('actions', $organization)
                @hasleadsenabled
                    @can('create crm leads')
                        <a href="{{ url(route('laravel-crm.leads.create',  ['model' => 'organization', 'id' => $organization->id])) }}"><button class="btn btn-sm btn-outline btn-rectangle"><x-mary-icon name="o-arrow-right" /><x-mary-icon name="fas.crosshairs" /></button></a>
                    @endcan
                @endhasleadsenabled
                @hasleadsenabled
                    @can('create crm deals')
                        <a href="{{ url(route('laravel-crm.deals.create',  ['model' => 'organization', 'id' => $organization->id])) }}"><button class="btn btn-sm btn-outline btn-rectangle"><x-mary-icon name="o-arrow-right" /><x-mary-icon name="fas.dollar-sign" /></button></a>
                    @endcan
                @endhasleadsenabled
                @hasquotesenabled
                    @can('create crm quotes')
                        <a href="{{ url(route('laravel-crm.quotes.create',  ['model' => 'organization', 'id' => $organization->id])) }}"><button class="btn btn-sm btn-outline btn-rectangle"><x-mary-icon name="o-arrow-right" /><x-mary-icon name="fas.file-lines" /></button></a>
                    @endcan
                @endhasquotesenabled
                @hasordersenabled
                    @can('create crm orders')
                        <a href="{{ url(route('laravel-crm.orders.create',  ['model' => 'organization', 'id' => $organization->id])) }}"><button class="btn btn-sm btn-outline btn-rectangle"><x-mary-icon name="o-arrow-right" /><x-mary-icon name="fas.shopping-cart" /></button></a>
                    @endcan
                @endhasordersenabled
