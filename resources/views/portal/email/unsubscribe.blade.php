...
<body>
    <div class="card">
        ...
        <p>{{ __('laravel-crm::lang.unsubscribe_prompt') }}</p>
        <p class="email">{{ $recipient->address }}</p>
        <form method="POST" action="{{ route('laravel-crm.email-tracking.unsubscribe.confirm', ['token' => $token]) }}">
