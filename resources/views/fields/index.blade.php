                            ...
                            @foreach($fields as $field)
                                <tr class="has-link" data-url="{{ url(route('laravel-crm.fields.show',$field)) }}">
                                    ...
                                    <td class="disable-link text-right">
                                        ...
                                        @endcan
                                        @can('edit crm fields')
                                            <a href="{{  route('laravel-crm.fields.edit', $field) }}" class="btn btn-outline-secondary btn-sm"><span class="fa fa-edit" aria-hidden="true"></span></a>
                                        @endcan
                                        @can('delete crm fields')
                                        <form action="{{ route('laravel-crm.labels.destroy',$field) }}" method="POST" class="form-check-inline mr-0 form-delete-button">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
