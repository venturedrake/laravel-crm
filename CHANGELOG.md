...
### Changed
- **Settings → General is now split across tabs** rather than one flat column of ~27 unrelated controls. The page grouped organisation identity, localisation, tax defaults, seven ID prefixes and five blocks of document terms into a single card, so finding "invoice terms" meant scrolling past everything else — and it only grew as document settings accumulated. It now uses the same DaisyUI tab strip as **Settings → Templates**, with six panels: **General** (identity, localisation, tax defaults, the related-activity toggle, and the phone / email / address panels), **Record IDs** (the lead, deal, order and delivery prefixes — the entities whose only setting is a prefix), **Documents** (the shared `pdf_contact_details` block and the dynamic-products toggle, both of which apply across document types), and one tab each for **Quotes**, **Invoices** and **Purchase orders** holding that type's ID prefix beside its own terms. Tabs are query-string-synced, so `/crm/settings?tab=invoices` deep-links; an unrecognised or module-disabled tab falls back to General. Each tab is hidden when its module is off, and a validation failure jumps to the tab holding the offending field rather than leaving Save looking inert
